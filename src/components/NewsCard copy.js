...
function NewsCard(props) {
    const {title, imagesrc, description, name, url, likes, comment, views, postId, baseURL} = props;

    const addLike = async (e) => {
        await axios.put(`${baseURL}/posts/addLike/${postId}?userId=614f4d55d35c933f145fa99a`).then((response) => {
    ...
    const removeLike = async (e) => {
        await axios.put(`${baseURL}/posts/removeLike/${postId}?userId=614f4d55d35c933f145fa99a`).then((response) => {
            console.log(response)
        });
        // eslint-disable-next-line no-restricted-globals
        location.reload()
    ...
    const Like = async (e) => {
        e.preventDefault()
        const user = await axios.get(`${baseURL}/admin-users/aUser/614f4d55d35c933f145fa99a`)
        ...
        <div>
           ...
           <div className="NewsCardWrapper">
              ...
              <div className="d-flex col-6 justify-content-between"> 
                  <div onClick={Like}> <img className="icons" src={heart}  alt="" />  <small> {likes} </small> </div>  
                  <div> <img className="icons" src={comments}  alt="" />  <small> {comment} </small> </div>  
                  <div> <Eye /> <small> {views} </small> </div>  
              </div>         
